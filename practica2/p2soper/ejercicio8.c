...
int hijo_pid;     
int rootPadre;    /*!<PID del padre raiz */

/**
 *@file ejercicio8.c
 *@autor victor garcia victor.garciacarrera@estudiante.uam.es, alfonso carvajal alfonso.carvajal@estudiante.uam.es
 *@brief Programa que crea un numero dado de procesos en serie. Estos procesos se mandaran señales
 * de forma ciclica hasta que se cumplan un numero de vueltas. El proceso raiz entonces mandara
 * una señal de terminacion a su hijo, que llegara hasta el ultimo.
 *@param argv[0] el nombre del programa
 *@param argv[1] que sera un entero positivo, el numero de procesos
 *@param argv[2] que sera un entero positivo, el numero de vueltas
 *@date: 17/03/2017
 *Grupo:2201 Pareja 4
 *
*/ 
...
void main(int argc, char **argv){
    ...
    for (j = 0; j < numvuel+1; j++){ /*!<Bucle que se encarga del paso de testigo circular*/
        ...
        if(signal (SIGUSR1, manejador_SIGUSR1)==SIG_ERR){	/*!<Armamos las señales*/
            perror("signal");
            exit(EXIT_FAILURE);
        }
